...
class MainWindowView(QMainWindow):
    ...
    def resizeEvent(self, event):
        """Invokes after window size change"""
    ...
    def select_image_file(self)->str:
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, \
            "Open Image File", "", "Image Files (*.png *.jpg *.jpeg)")
    ...
    def save_image_file(self)->str:
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, \
            "Save Image File", self.__file_name, \
    ...
    def display_image(self, image):
        ...
        height, width, channels_number = image.shape \
            if len(image.shape) == 3 else (*image.shape, 1)
        ...
        pixmap = pixmap.scaled(self.scrollArea.width() - 30, \
            self.scrollArea.height() - 30, \
    ...
    def get_margin_size(self):
        ...
        return (self.upSB.value(),self.rightSB.value(), \
            self.bottomSB.value(),self.leftSB.value())

    def get_line_params(self):
        """Returns coordinates (x1, y1, x2, y2)
        of the segment ends and brush thickness"""
        try:
            params = (int(self.x1LE.text()),int(self.y1LE.text()), \
                int(self.x2LE.text()),int(self.y2LE.text()))
            for param in params:
                if param < 0:
                    raise ValueError
    ...
    def add_line_to_list(self, _x1, _y1, _x2, _y2, thickness):
        """Adds line to linesLst with specified params:
        x1, y1, x2, y2, thickness"""
        item = QListWidgetItem(f"Line: (x1: {_x1}, y1: {_y1}, "
                               f"x2: {_x2}, y2: {_y2}, "
